...
class Network(object):
    ...
    def SGD(self, training_data, epochs, mini_batch_size, learning_rate,
        ...
        outputs.
        Optional (commented out): Returns vectors of training accuracy, loss and test accuracy across epochs
        """
    ...
    def backprop(self, x, y):
        ...
        #calculated a bit differently than shown on lecture
        Z, A = self.forward_pass(x)
        L = self.num_layers
        deltas = [0] * (L + 1)
        deltas[-1] = self.loss_derivative_wr_output_activations(A[-1], y)
        i = L - 1
        while i >= 2:
            deltas[i] = np.multiply(np.dot(self.weights[i - 1].T, deltas[i + 1]), relu_derivative(Z[i - 1]))
            i -= 1
        db = []
        dw = []
        for i in range(L - 1):
            db.append(deltas[i + 2])
            dw.append(np.outer(deltas[i + 2], A[i]))
    ...
    def forward_pass(self, x):
        ...
        for i in range(self.num_layers - 1):
            v_cur = np.dot(self.weights[i], z_cur) + self.biases[i]
            z_cur = relu(v_cur)
            V.append(v_cur)
            Z.append(z_cur)
        return V, Z
    ...
    def loss(self, data):
        ...
        for (x, y) in data:
            ...
            loss_list.append(-np.log(np.dot(net_output_after_softmax.transpose(), y).flatten()[0]))
    ...
    def output_softmax(self, output_activations):
        ...
        tmpvec = output_activations - np.max(output_activations) #improves numerical stability
        return np.exp(tmpvec) / np.sum(np.exp(tmpvec))
